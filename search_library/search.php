...
//         $input_new = preg_replace("/\b[-0-9a-zA-Z.+_]+@[-0-9a-zA-Z.+_]+.[a-zA-Z]{2,4}\b/",'',$input_new);  
        ...
        $data['string']=(!empty($email))?'':$expert_and_company; 
//         array_push($data['string'], $email); 
        $data['email']=$email;    
        if($data['string']='' AND $data['email']='')
        {
            echo "string";
        }
        ...
        {   
            ...
            if(!empty($data['email']))
            {
                if ($value_q['type']=='email'||true) 
                {   
                    $query='SELECT '.$value_q['get_colms'].' FROM '.$value_q['table_name'].' WHERE email ="'.$email.'"';
                    array_push($query_array, $query);
                    array_push($get_ids,$value_q['get_id']); 
                }
            }
