...
//     die(json_encode(array("status"=>true)));
	...
	{   
	    ...
	    $keys=array('type','table_name','search_col_name','email','get_colms','get_id');
	    $value=array(array('string','login_db.mock_test_tbl','name','email','null as name,id,null as email,null as phone,null as gender','id'));
	    $query_data=array();
	...
	Class ManageApp {
		...
		function MyConnection (&$connection,$host,$user,$db)
		{
			...
			$connection= mysqli_connect ($host, $user, "Root@123" , $db); 
			if (!$connection) 
			{
